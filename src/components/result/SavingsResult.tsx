    ...
    datasets: [
      {
        ...
        data: [
          ...
          result[`totalPaidIn${selectedPeriod}Year${multipleYears}`] -
            result[`savedIn${selectedPeriod}Year${multipleYears}`],
        ],
        backgroundColor: ["rgba(188, 0, 45, 0.9)", "rgba(0, 188, 125, 0.9)"],
        borderColor: ["rgba(188, 0, 45, 1)", "rgba(0, 188, 125, 1)"],
        borderWidth: 1,
      },
    ],
  };
    ...
    <div className="space-y-6 p-8 bg-white shadow-md rounded-lg">
      ...
      <div className="text-center">
        <p className="text-xl font-bold text-gray-800">
          Você pagaria um total de{" "}
          <strong className="text-red-600">
            {formatCurrency(textData.total)}
          </strong>
        </p>
        <p className="text-xl font-bold text-gray-800">
          Com a Clean Energy, você paga:{" "}
          <strong className="text-emerald-500">
            {formatCurrency(textData.total - textData.saved)}
          </strong>
        </p>
        <p className="mt-4 text-3xl font-extrabold text-emerald-500">
          {" "}
          Sua economia em {selectedPeriod} ano{multipleYears} é de{" "}
          {formatCurrency(textData.saved)}
        </p>
      </div>
      ...
      <div className="flex justify-center space-x-2">
        <button
          ...
          className={`px-4 py-2 text-sm font-medium rounded-md ${
            selectedPeriod === 1 ? "bg-emerald-500 text-white" : "bg-gray-200"
          }`}
        >
          1 Ano
        </button>
        <button
          ...
          className={`px-4 py-2 text-sm font-medium rounded-md ${
            selectedPeriod === 3 ? "bg-emerald-500 text-white" : "bg-gray-200"
          }`}
        >
          3 anos
        </button>
        <button
          ...
          className={`px-4 py-2 text-sm font-medium rounded-md ${
            selectedPeriod === 5 ? "bg-emerald-500 text-white" : "bg-gray-200"
          }`}
        >
          5 anos
        </button>
