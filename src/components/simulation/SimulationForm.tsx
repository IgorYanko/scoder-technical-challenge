...
const leadSchema = z.object({
   ...
   monthlyBillValue: z.number({ required_error: "O valor da conta é obrigatório." }).positive("O valor deve ser positivo."),
   supplyType: z.enum(['MONOPHASIC', 'BIPHASIC', 'TRIPHASIC'], { required_error: "Selecione o tipo."}),
...
export default function SimulationForm({ onSuccess }: SimulationFormProps) {
   ...
   useEffect(() => {
      ...
      fetch(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${watchedState}/municipios`)
      .then(res => res.json())
      .then(data => {
         setCities(data);
      });
   ...
   return (
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 p-8 bg-white shadow-md rounded-lg">
        <h2 className="text-2xl font-bold text-gray-800 text-center">Venha conhecer o futuro da energia limpa!</h2>
        ...
        <div>
            ...
            {errors.supplyType && <p className="mt-2 text-sm text-red-600">{errors.supplyType.message}</p>}
        </div>
